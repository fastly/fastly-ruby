...
opts = {
    ...
    keepalive_time: 56, # Integer | How long (in seconds) to keep a persistent connection to the backend between requests. By default, Fastly keeps connections open as long as it can.
    max_conn: 56, # Integer | Maximum number of concurrent connections this backend will accept.
    max_lifetime: 56, # Integer | Maximum time from creation (in milliseconds) that a pooled HTTP keepalive connection will be eligible for reuse; 0 is treated as unlimited.
    ...
    max_use: 56, # Integer | Maximum number of requests allowed over a single, pooled HTTP keepalive connection to this backend; 0 is treated as unlimited.
