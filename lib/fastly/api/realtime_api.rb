...
module Fastly
  class RealtimeApi
    ...
    # Get data for the 120 seconds preceding the latest timestamp available for a service.
    # @option opts [String] :service_id Alphanumeric string identifying the service. (required)
    # @return [Array<(Realtime, Integer, Hash)>] Realtime data, response status code and response headers
    ...
    # @option opts [String] :service_id Alphanumeric string identifying the service. (required)
    # @option opts [Integer] :timestamp_in_seconds Timestamp in seconds (Unix epoch time). (required)
    # @return [Realtime]
