  ...
  class LoggingBigqueryResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Fastly::LoggingBigqueryResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Fastly::LoggingBigqueryResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'template_suffix')
        self.template_suffix = attributes[:'template_suffix']
      end
    ...
    def hash
      [name, placement, format_version, response_condition, format, user, secret_key, account_name, dataset, table, template_suffix, project_id, created_at, deleted_at, updated_at, service_id, version].hash
    end
