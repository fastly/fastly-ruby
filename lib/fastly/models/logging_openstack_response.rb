  ...
  class LoggingOpenstackResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Fastly::LoggingOpenstackResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Fastly::LoggingOpenstackResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [name, placement, format_version, response_condition, format, message_type, timestamp_format, period, gzip_level, compression_codec, access_key, bucket_name, path, public_key, url, user, created_at, deleted_at, updated_at, service_id, version].hash
    end
