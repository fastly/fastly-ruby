  ...
  class WafFirewallResponseDataAttributesAllOf
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Fastly::WafFirewallResponseDataAttributesAllOf` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Fastly::WafFirewallResponseDataAttributesAllOf`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [service_id, active_rules_fastly_block_count, active_rules_fastly_log_count, active_rules_fastly_score_count, active_rules_owasp_block_count, active_rules_owasp_log_count, active_rules_owasp_score_count, active_rules_trustwave_block_count, active_rules_trustwave_log_count].hash
    end
